...
function love.load()
    ...
    local possibleFoodPositions = {}
        ...
        foodObstacle = possibleFoodPositions[love.math.random(2, #possibleFoodPositions)]
        foodObstacle2 = possibleFoodPositions[love.math.random(3, #possibleFoodPositions)]
    ...
    function reset()
        ...
        gui.timer = 0;
        gameTimer = 0;
        if bestGameScore < gameScore then
    ...
    if snakeAlive then
        ...
        if timer >= timerLimit then
            ...
            if canMove then
                ...
                elseif snakeSegments[1].x == foodObstacle.x
                and snakeSegments[1].y == foodObstacle.y then
                    moveFood();
                    gameScore = gameScore - 1;
                elseif snakeSegments[1].x == foodObstacle2.x
                and snakeSegments[1].y == foodObstacle2.y then
                    moveFood();
...
function love.draw()
    ...
    love.graphics.setColor(0, 1, 0); -- food colour
    drawCell(foodPosition.x, foodPosition.y);

    love.graphics.setColor(1, 1, 0); -- food colour
    drawCell(foodObstacle.x, foodObstacle.y);

    love.graphics.setColor(1, 0, 0); -- food colour
    drawCell(foodObstacle2.x, foodObstacle2.y);
...
function drawGUI()
  ...
  love.graphics.print("Score: " .. tostring(gameScore), 50, 425);
  love.graphics.print("Timer: " .. tostring(gui.timer), 270, 425);
  love.graphics.print("Best Score: " .. tostring(bestGameScore), 450, 425);
